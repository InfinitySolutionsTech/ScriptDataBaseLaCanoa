-- SQL Schema
CREATE SCHEMA lasburguers;
-- Create Catalogs table (possible roles)
CREATE TABLE lasburguers.catalogs (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    role VARCHAR(50) NOT NULL UNIQUE
);
-- Create IdentificationType table
CREATE TABLE lasburguers.identificationType (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    name VARCHAR(50) NOT NULL UNIQUE
);
CREATE TABLE lasburguers.users (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE lasburguers.product_type (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    name VARCHAR(50) NOT NULL UNIQUE,
    parent_id BIGINT,
    color_hex VARCHAR(7)
...
CREATE TABLE lasburguers.product (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE lasburguers.tables (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    name VARCHAR(50) NOT NULL
);
-- Create PaymentMethod table
CREATE TABLE lasburguers.payment_method (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    name VARCHAR(255) NOT NULL
);
-- Create Orders table
CREATE TABLE lasburguers.orders (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE lasburguers.order_details (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE lasburguers.reports (
...
CREATE TABLE lasburguers.supplierCategories (
...
CREATE TABLE lasburguers.suppliers (
    ...
    FOREIGN KEY (createdBy) REFERENCES lasburguers.users(id)
);

-- Clientes 
CREATE TABLE lasburguers.personTypes (
...
CREATE TABLE lasburguers.clients (
    ...
    FOREIGN KEY (identificationTypeID) REFERENCES lasburguers.identificationType(id),
    FOREIGN KEY (personTypeID) REFERENCES lasburguers.personTypes(PersonTypeID),
    FOREIGN KEY (createdBy) REFERENCES lasburguers.users(id)
);
...
CREATE TABLE lasburguers.invoice (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    order_id BIGINT NOT NULL,
    client_id BIGINT,
    ...
    CONSTRAINT fk_invoice_client FOREIGN KEY (client_id) REFERENCES lasburguers.clients(clientID)
);
-- Gastos
-- Tabla complementaria para categorías de gastos
CREATE TABLE lasburguers.expense_categories (
...
CREATE TABLE lasburguers.expenses (
    ...
    FOREIGN KEY (supplierID) REFERENCES lasburguers.suppliers(supplierID),
    FOREIGN KEY (created_by) REFERENCES lasburguers.users(id),
    FOREIGN KEY (payment_method_id) REFERENCES lasburguers.payment_method(id),
    FOREIGN KEY (expense_category_id) REFERENCES lasburguers.expense_categories(categoryID)
);

CREATE TABLE lasburguers.notification_type(
...
CREATE TABLE lasburguers.notification_history(
...
CREATE TABLE lasburguers.notification_user(
